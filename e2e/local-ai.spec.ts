...
test.describe("Local AI E2E Flows", () => {
  ...
  test("D. Invoice scan: requires local AI vision model when AI is not active", async ({ page }) => {
    await setupApp(page);
    ...
    await expect(dialog.getByText("Skanowanie faktur wymaga włączenia lokalnego AI i modelu Ollama vision.")).toBeVisible();
