...
export interface DebtPayoffSimulatorResult {
  ...
  monthsSaved: number;
  snowballQueue: { name: string; amount: number }[];
  isDebtFree: boolean;
}
...
export function calculateDebtPayoffSimulator(
  ...
  const payments = Array.isArray(profile.payments) ? profile.payments : [];
  for (const p of payments) {
    if (p.status !== "Opłacono" && p.amount > 0) {
      debtItems.push({
        id: p.id,
  ...
  const baseMonthlyPayment = monthlyAvailableSurplus > 0 ? monthlyAvailableSurplus : 300;

  const baselineMonths = totalDebt > 0
    ? Math.ceil(totalDebt / baseMonthlyPayment)
    : 0;

  const totalAcceleratedMonthly = baseMonthlyPayment + Math.max(0, extraPayment);
  const acceleratedMonths = totalDebt > 0
    ? Math.ceil(totalDebt / totalAcceleratedMonthly)
    : 0;
