    ...
    <div className="flex flex-col gap-4 w-full">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4" id="widget-content-stats-grid">
        ...
        <div className="bg-surface border border-border rounded-2xl p-5 relative shadow-sm hover:bg-surface transition group min-w-0">
          ...
          <h2 className={`text-2xl font-bold mb-1 relative z-10 truncate pr-10 ${balance >= 0 ? "text-brand" : "text-danger"}`} id="dash-balance-total" title={formatMoney(balance, currency)}>
          ...
          <div className="flex flex-col gap-1 mt-2 relative z-10 min-w-0">
            {emergencyLimit > 0 && (
              <div className="text-xs bg-brand-subtle text-brand px-2.5 py-1 rounded-md border border-brand/20 font-bold w-fit tracking-wide shadow-sm truncate max-w-full" title={`Limit awaryjny: ${formatMoney(emergencyLimit, currency)}`}>
                Limit awaryjny: {formatMoney(emergencyLimit, currency)}
              </div>
            )}
            {investmentCushion > 0 && (
              <div className="text-xs bg-surface-2 text-text-muted px-2.5 py-1 rounded-md border border-border font-bold w-fit tracking-wide shadow-sm truncate max-w-full" title={`Poduszka fin.: ${formatMoney(investmentCushion, currency)}`}>
        ...
        <div className="border border-border bg-surface rounded-2xl p-5 shadow-sm relative overflow-hidden group hover:bg-surface transition min-w-0">
          ...
          <div className="flex items-start justify-between relative z-10 min-w-0">
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 mb-1 min-w-0">
                ...
                <span className="bg-surface-2 border border-border text-text-muted text-xs font-bold px-2 py-0.5 rounded-full uppercase tracking-wider shrink-0">AI Auto-Calc</span>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4 pt-4 border-t border-border relative z-10">
            <div className="bg-surface p-3.5 rounded-xl border border-border shadow-inner min-w-0 flex flex-col">
              ...
              <span className="text-lg font-bold text-danger truncate" title={formatMoney(endOfMonthForecast.unpaidPaymentsSum + endOfMonthForecast.futureRecurringExpensesSum, currency)}>{formatMoney(endOfMonthForecast.unpaidPaymentsSum + endOfMonthForecast.futureRecurringExpensesSum, currency)}</span>
            </div>
            ...
            <div className="bg-surface p-3.5 rounded-xl border border-border shadow-inner min-w-0 flex flex-col">
              ...
              <span className="text-lg font-bold text-brand truncate" title={formatMoney(endOfMonthForecast.futureRecurringIncomesSum, currency)}>{formatMoney(endOfMonthForecast.futureRecurringIncomesSum, currency)}</span>
            </div>
            ...
            <div className="bg-surface-2 p-3.5 rounded-xl border border-border min-w-0 flex flex-col">
              ...
              <span className={`text-lg font-black truncate ${endOfMonthForecast.forecastedBalance >= 0 ? "text-brand" : "text-danger"}`} title={formatMoney(endOfMonthForecast.forecastedBalance, currency)}>
          ...
          {endOfMonthForecast.isNegative && (
            <div className="mt-3.5 bg-danger-subtle border border-danger/20 rounded-xl p-3 text-xs text-danger flex items-start gap-2 animate-pulse relative z-10 min-w-0">
        ...
        >
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative z-10">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-bold active:scale-[0.98] transition-all whitespace-nowrap shadow-sm border shrink-0 focus-visible:ring-2 cursor-pointer ${
                safeBreakdown.isNegative
                ? "bg-danger-subtle text-danger hover:bg-danger-subtle border-danger/20 focus-visible:ring-focus-ring"
                : "bg-brand-subtle text-brand hover:bg-brand-subtle border-brand/20 focus-visible:ring-focus-ring"
              }`}
            >
              Zobacz Analizę →
            </button>
          ...
          <div className="mt-5 pt-4 border-t border-border flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10 min-w-0">
            <div className="min-w-0 flex-1">
              ...
              <span className={`text-2xl sm:text-3xl font-black truncate block ${safeBreakdown.isNegative ? "text-danger" : "text-brand"}`} title={formatMoney(safeBreakdown.safeToSpend, currency)}>
