    ...
    <div className="bg-surface p-5 rounded-2xl border border-border shadow-sm flex flex-col justify-between h-full relative overflow-hidden" id="widget-content-activity-box">
      ...
      <div className="flex items-center justify-between mb-5 relative z-10 gap-4 min-w-0">
        <div className="min-w-0 flex-1">
          <p className="text-[10px] font-bold text-text-muted uppercase tracking-wider mb-0.5 truncate" title="Ostatnie transakcje">Ostatnie transakcje</p>
          <h3 className="text-base font-bold text-text-main truncate" title="Aktywność">Aktywność</h3>
        </div>
        <button
          onClick={() => onChangeView("transactions")}
          className="text-[10px] font-bold text-purple-300 bg-purple-500/10 border border-purple-500/20 px-2.5 py-1.5 rounded-lg hover:bg-purple-500/20 active:scale-[0.98] transition-all shrink-0 truncate max-w-[100px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-500/50 cursor-pointer"
          title="Księga"
        >
          Księga
        </button>
                ...
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5 min-w-0">
                      <span className="text-xs font-bold text-text-main truncate group-hover:text-purple-300 transition-colors" title={t.name || t.category}>
                        {t.name || t.category}
                      </span>
                      {t.paidBy && (
                        <span 
                    ...
                    <p className="text-[9px] text-text-muted mt-0.5 truncate max-w-[100px]" title={formatDate(t.isoDate)}>{formatDate(t.isoDate)}</p>
                  </div>
                </div>
                <div className="text-right shrink-0 pl-2 min-w-0">
                  <p className={`text-xs font-black truncate max-w-[100px] ${t.type === 'income' ? 'text-emerald-700 drop-' : 'text-text-main'}`} title={t.type === 'income' ? `+${formatMoney(t.amount, t.currency || currency)}` : `-${formatMoney(t.amount, t.currency || currency)}`}>
      ...
      <div className="pt-4 mt-4 border-t border-border relative z-10">
        <button
          ...
          className="w-full py-2.5 bg-surface hover:bg-surface-offset active:scale-[0.98] transition-all text-text-main text-xs font-bold rounded-xl border border-border shadow-inner flex items-center justify-center min-w-0 shrink-0 truncate px-2 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/50"
          title="Szybki zapis"
        >
