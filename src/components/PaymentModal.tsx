...
export function PaymentModal({ isOpen, onClose, initialData, onSave }: PaymentModalProps) {
  ...
  return (
    ...
    >
      ...
      >
        <button onClick={onClose} className="absolute top-4 right-4 text-2xl text-slate-400 hover:text-slate-300" id="close-payment-modal">
          &times;
        </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="border-t border-slate-700/50 pt-3">
              {!activeProfile.partnerName ? (
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-400 mb-1">Kto płaci?</label>
                    <select
                      value={paidBy}
          ...
          <button
            ...
            className="w-full rounded-xl bg-slate-900 py-3 text-sm font-bold text-white shadow-lg hover:bg-slate-900/95 backdrop-blur-2xl transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
