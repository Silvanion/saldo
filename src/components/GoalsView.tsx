        ...
        {profile.goals.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4" id="goals-grid">
            {profile.goals.map((g) => {
              ...
              if (isCompleted) {
                badgeInfo = { text: "Osiągnięty 🎉", colorClass: "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" };
              } else if (percent >= 90) {
                badgeInfo = { text: "Prawie u celu!", colorClass: "bg-amber-500/10 text-amber-400 border-amber-500/20" };
              } else if (percent > 0) {
              ...
              return (
                <div key={g.id} className="bg-surface border border-border rounded-2xl p-5 shadow-lg hover:shadow-xl hover:bg-surface transition flex flex-col justify-between h-[13rem] relative">
                  <div>
                    ...
                    {paceText && (
                      <p className="text-[10px] text-amber-400 mt-1.5 font-semibold bg-amber-500/10 inline-block px-2 py-1 rounded-md border border-amber-500/20">
                        {paceText}
                      </p>
                    )}
                  </div>
                  <div>
                    {/* Progress */}
                    <div className="w-full bg-slate-700/50 h-2 rounded-full overflow-hidden mt-3 mb-1">
                      <div
                        style={{ width: `${percent}%` }}
                  ...
                  [...profile.investments]
                    ...
                    .map((inv) => (
                      <div key={inv.id} className="flex justify-between items-center py-3">
                        ...
                        <strong className="text-sm text-emerald-400">{formatMoney(inv.amount, inv.currency || profile?.currency || 'PLN')}</strong>
                      </div>
                    ))
                )}
              </div>
